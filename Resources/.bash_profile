...
curl -s $1 | grep path | sed -n "s/.*resource path=\"\(.*\)\".*/\1/p" | tee -a /opt/RedTeam-ToolKit/Web/dirsearch/db/yahooapi.txt
}

#----- recon -----
crtndstry(){
/opt/RedTeam-ToolKit/Web/crtndstry/crtndstry $1
}
...
python3 /opt/RedTeam-ToolKit/Web/dirsearch/dirsearch.py -u $1 -e $2 -t 50 -b 
}

sqlmap(){
python /opt/RedTeam-ToolKit/Web/sqlmap*/sqlmap.py -u $1 
}
...
curl -s https://crt.sh/?q\=%.$1\&output\=json | jq -r '.[].name_value' | sed 's/\*\.//g' | sort -u | httprobe -c 50 | grep https | xargs -n1 -I{} python3 /opt/RedTeam-ToolKit/Web/dirsearch/dirsearch.py -u {} -e $2 -t 50 -b 
}
